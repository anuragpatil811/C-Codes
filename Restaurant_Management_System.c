...
struct node* createadmin(struct node *head,int data, char foodname[25],
 float price)
{
...
void displayList(struct node *head)
{
    ...
    else
    {
        ...
        while(temp1!=NULL)
        {
            ...
            {
                printf("\t\t\t\t\t\t\t%d\t%s\t%d\t%0.2f\n",temp1->data,
				temp1->foodname,temp1->quantity,temp1->price);
            }
...
struct node* deleteList(struct node* head)
{
    ...
    else
    {
        ...
        while(temp->next!=0)
        {
            temp = temp->next; //It enters a loop that iterates until temp->next is NULL, i.e., until temp reaches the last node of the list.
	 //In each iteration of the loop,It enters a loop that iterates until temp->next is NULL, i.e., until temp reaches the last node of the list.
            free(temp->prev);
        }
        free(temp);
        head = NULL;
